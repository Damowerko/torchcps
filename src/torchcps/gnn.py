...
class GIN(nn.Module):
    @staticmethod
    def add_args(parser: argparse.ArgumentParser):
        group = parser.add_argument_group(GIN.__name__)
        group.add_argument(
            "--n_channels",
            type=int,
            default=32,
        ...
        group.add_argument(
            ...
            type=int,
            default=1,
            help="Number of MLP layers to use for the GIN architecture.",
        )
        group.add_argument(
            "--dropout", type=float, default=0.0, help="Dropout probability."
        )
